<template>
    <div> 
        <el-row>
            <el-col :span="6">
                <el-form>
                    ...
                    <el-form-item>
                        <el-input v-model="filterText" placeholder="检索事件分类" clearable></el-input>
                    </el-form-item>
                </el-form>
            ...
            <el-col :span="18" style="padding-left:10px;">
				 <el-form inline ref="searchForm" :model="searchForm">
					<el-form-item label="事件分类名称" prop="group_name">
		...
		<!-- 事件分类编辑 -->
        <el-dialog title="事件分类" :visible.sync="dialogInfo" :modal-append-to-body="false" :close-on-click-modal="false" width="35%">
            <el-form :model="ruleForm" :rules="this.$validateRule" ref="ruleForm" label-width="110px" class="event_group_from">
				...
				<el-form-item label="事件分类层级" prop="group_level">
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="saveEGInfo('ruleForm')">确 定</el-button>
            </div>
        </el-dialog>
...
export default {
  ...
  methods: {
	...
	handleAdd(){
		...
		this.dialogStatus.title = '新增事件分类';
		...
		if(this.clickItem&&this.clickItem.id){
            ...
            this.itemshow = false;
            if(this.clickItem.children){//存在子节点
    ...
    handleEdit(index, row) {
		...
		this.itemshow = false;
		this.dialogInfo = true;
	  	this.dialogStatus.title = '编辑事件分类';
    ...
    delEGInfo(data) {
      const _vm = this;
      this.$confirm("此操作将永久删除此事件分类, 是否继续?", "提示", {
        ...
        .then(async () => {
            const res = await _vm.$http.post(
              "/cms/EventGroupController/delEGInfo",
              data
            );
            if (res.data && res.data.success) {
                this.clickItem = {};
                _vm.getEventGroupList();
                _vm.getEventGroupAll();
                _vm.$message.success("事件分类删除成功");
            } else {
    ...
    async getEventGroupList() {
        ...
        const res = await this.$http.get(
            "/cms/EventGroupController/eventGroupList",
            { params: data }
        );
        if (res.data && res.data.success) {
	 ...
	 * 获取全部事件分类列表组装树结构
	 */
    async getEventGroupAll() {
        this.treeData = [];
        const res = await this.$http.get("/cms/EventGroupController/eventGroupAll");
        if (res.data && res.data.success) {
            const data = res.data.result.map((item)=>{
                item.show = false;
                return item;
            });
            this.treeData = transData(res.data.result,'id','parent_id','children') ;
        } else {
	...
	*/
    ...
    treeNodeDel(){
        if(this.clickItem&&this.clickItem.id){
            if(this.clickItem.children){
                this.$message.warning('当前事件分类包含小类，请先删除小类');
            }else{
                this.delEGInfo({id:this.clickItem.id});
            }
        }else{
            this.$message.warning('请先选中事件分类节点');
        }
	...
	saveEGInfo(formName){
		...
		this.$refs[formName].validate(async valid => {
			if (valid) {
                const res = await _vm.$http.post(
                "/cms/EventGroupController/saveEGInfo",
                formData
                );
                if (res.data && res.data.success) {
                    this.clickItem = {};
