  ...
  <div>
    <el-form>
      ...
      <el-form-item style="margin-bottom: 5px;">
        <el-select v-model="eventParams.parent_name" placeholder="案件类型" style="width: 150px;">
    ...
    <el-dialog class="childDialog" v-bind:title="formTitle" :visible.sync="dialogInfo" :modal-append-to-body="false"
      ...
      <el-form label-width="110px">
        ...
        <el-form-item label="案卷编号:">
          <span>{{detail.case_code !== '' ? detail.case_code : '无'}}</span>
    ...
    methods: {
      ...
      async allEventList() {
        ...
        try {
          ...
          let result = await _this.$http.post('/oa/OaCaseController/get_all_case_list', param);
      ...
      async onEventSearch() {
        ...
        try {
          this.all_event_list = [];
          let result = await this.$http.post('/oa/OaCaseController/get_specific_case_list', params);
          if (result.data.success) {
            let data = result.data.result.rows;
      ...
      async del_event_list(data){
        ...
        _this.$confirm('此操作将永久删除已勾选事件, 是否继续?', '提示', {
          ...
          try {
            let res = await _this.$http.post('/oa/OaCaseController/delete_case', data);
      ...
      async handleDelete(index, row) {
        ...
        _this.$confirm('此操作将永久删除该条事件, 是否继续?', '提示', {
          ...
          try {
            let res = await _this.$http.post('/oa/OaCaseController/delete_case', param);
            if (res.data && res.data.success) {
              _this.$message.success('删除成功');
              _this.allEventList();
