<template>
    <div class="job-cjy">
		<el-form :inline="true" :model="searchForm" ref="searchForm">
			...
			<el-form-item>
				<el-button  type="primary" @click="getCenterPerEvalList">评价</el-button>
			</el-form-item> 
			<el-form-item>
		...
		<el-table
			...
			<el-table-column label="派遣核实及时率">
				<el-table-column
					prop="verifyEventCount"
					label="派遣核实次数">
				</el-table-column>
				<el-table-column
					prop="onVECount"
				...
				<el-table-column
					...
					<template slot-scope="scope">
						<div v-if="scope.row.VERate!==null"> {{scope.row.VERate+'%'}}</div>
						<div v-else>--</div>
					</template>
				</el-table-column>
				<el-table-column
					label="级别">
					<template slot-scope="scope">
						<el-tag :disable-transitions='true' :color="gradeColor(scope.row.VERate)" class="el-tag-grade">
							{{gradeLevel(scope.row.VERate)}}
						</el-tag>
					</template>
				</el-table-column>
			</el-table-column>
			<el-table-column label="案卷批转及时率">
				<el-table-column
					prop="taskCount"
					label="批转案卷数">
				</el-table-column>
				...
				<el-table-column
					label="案卷批转及时率">
					<template slot-scope="scope">
						<div v-if="scope.row.taskRate!==null"> {{scope.row.taskRate+'%'}}</div>
						<div v-else>--</div>
					</template>
				</el-table-column>
				<el-table-column
					label="级别">
					<template slot-scope="scope">
						<el-tag :disable-transitions='true' :color="gradeColor(scope.row.taskRate)" class="el-tag-grade">
							{{gradeLevel(scope.row.taskRate)}}
						</el-tag>
					</template>
				</el-table-column>
			</el-table-column>
			<el-table-column label="事件立案及时率">
				<el-table-column
					prop="caseCount"
					label="发起立案次数">
				</el-table-column>
				...
				<el-table-column
					label="事件及时立案率">
					<template slot-scope="scope">
						<div v-if="scope.row.ECRate!=null">{{scope.row.ECRate+'%'}}</div>
						<div v-else>--</div>
					</template>
				</el-table-column>
				<el-table-column
					label="级别">
					<template slot-scope="scope">
						<el-tag :disable-transitions='true' :color="gradeColor(scope.row.ECRate)" class="el-tag-grade">
							{{gradeLevel(scope.row.ECRate)}}
						</el-tag>
					</template>
				</el-table-column>
			</el-table-column>
		...
		<!--查询部门选择器-->
        <el-dialog title="人员选择" :visible.sync="dialogVisible" class="groupclass" :close-on-click-modal="false">
...
export default {
	...
	mounted() {
		...
		this.getAllCenterPer();
	},
	methods: {
		async getAllCenterPer(){//获取所有受理人员
			const data = {
			...
			if(res.data&&res.data.success){
				...
				if(list.length>0){
					...
					await this.getCenterPerEvalList();
				}
			}else{
		...
		async getCenterPerEvalList(){
			...
			const res = await this.$http.post('/overallmerit/JobController/getCenterPerEvalList',data);
			if(res.data&&res.data.success){
		...
		closeDialog(value) {
			if (value === 1) {
				if(this.departTreeChangeArr.length===0){
					this.$message.warning('请至少选择一个受理人员');
				}else{
